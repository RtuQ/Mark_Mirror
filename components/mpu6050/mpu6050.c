...
{
	return mpu6050_register_write_byte(MPU6050_RA_GYRO_CONFIG,fsr);//设置陀螺仪满量程范围  
}
...
{
	return mpu6050_register_write_byte(MPU6050_RA_ACCEL_CONFIG,fsr);//设置加速度传感器满量程范围  
}
...
esp_err_t mpu6050_i2c_write(unsigned char slave_addr, unsigned char reg_addr, unsigned char length, unsigned char const *data)
{
	...
	ret = i2c_master_write_to_device(0, slave_addr, write_buf, 1 + length, 1000 / portTICK_PERIOD_MS);
...
{
    return i2c_master_write_read_device(0, slave_addr, &reg_addr, 1, data, length, 1000 / portTICK_PERIOD_MS);
}
...
void __mpu6050_active_task(void* arg)
{
  float temp = 0;
  short mpu_acc[3];
  short mpu_gypro[3];
  ...
  while (1) {
    ...
    while(count < 200) {
      mpu6050_read_acc(&mpu_acc);
      AccX = mpu_acc[0] / 16384.0;
      AccY = mpu_acc[1] / 16384.0;
      AccZ = mpu_acc[2] / 16384.0;
    ...
    count = 0;
    while(count < 200) {
      mpu6050_read_gyro(&mpu_gypro);
      GyroX = mpu_gypro[0] / 131.0;
      GyroY = mpu_gypro[1] / 131.0;
      GyroZ = mpu_gypro[2] / 131.0;
    ...
    ESP_LOGI(TAG,"now temperature is %f,[Acc] %f %f %f x:%lf y:%lf,[Gyro] x:%f y:%f z:%f",temp,AccX,AccY,AccZ,AccErrorX,AccErrorY,GyroErrorX,
...
esp_err_t mpu6050_init(void) 
{
    ...
    ESP_LOGI(TAG,"[MPU6050]:who am I> %x is 0x68?",data);
    ...
    xTaskCreatePinnedToCore(__mpu6050_active_task, "mpu6050_active_task", 4096 * 2, NULL, 4, NULL, 0);
    return ESP_OK;
}